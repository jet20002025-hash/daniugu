...
def reanalyze_stocks():
    ...
    for stock_code, stock_name in stocks_to_analyze:
        ...
        print(f"\n📊 周K线数据概览（每10周显示一次）:")
        print("-" * 80)
        for i in range(0, len(weekly_df), 10):
            row = weekly_df.iloc[i]
            date = row['日期']
            close = row['收盘']
            volume = row.get('周成交量', row.get('成交量', 0))
            change_pct = row.get('涨跌幅', 0)
            print(f"   [{i:3d}] {date} | 收盘: {close:.2f} | 成交量: {volume:,.0f} | 涨跌幅: {change_pct:+.2f}%")
        ...
        if start_idx is not None:
            print(f"\n📊 当前买点前后各15周的数据:")
            print("-" * 80)
            start_range = max(0, start_idx - 15)
            end_range = min(len(weekly_df), start_idx + 16)
            ...
            for i in range(start_range, end_range):
                ...
                marker = " ⭐ 当前买点" if i == start_idx else ""
                print(f"   [{i:3d}] {date} | 收盘: {close:.2f} | 最高: {high:.2f} | 最低: {low:.2f} | 成交量: {volume:,.0f} | 涨跌幅: {change_pct:+.2f}%{marker}")
        
        # 尝试不同的search_weeks参数，看看是否能找到更好的买点
        print(f"\n🔍 尝试不同的查找窗口，寻找最佳买点...")
        for search_weeks in [5, 10, 15, 20]:
            print(f"\n   - 尝试 {search_weeks} 周窗口...")
            result = analyzer.find_max_gain_interval(stock_code, search_weeks=search_weeks, min_gain=50.0)
            if result.get('success'):
                interval_test = result.get('interval', {})
                gain_test = interval_test.get('涨幅', 0)
                start_date_test = interval_test.get('起点日期', '')
                start_price_test = interval_test.get('起点价格', 0)
                print(f"     找到涨幅: {gain_test:.2f}% | 起点: {start_date_test} | 价格: {start_price_test:.2f}")
