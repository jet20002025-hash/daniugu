# 🚀 完整部署指南 - daniugu.online

## 📋 部署信息

- **域名**: daniugu.online
- **GitHub**: https://github.com/jet20002025-hash/daniugu
- **服务器**: 阿里云服务器

## 🎯 部署步骤

### 第一步：将代码推送到 GitHub

#### 1. 在本地执行推送脚本

```bash
cd /Users/zwj/股票分析
bash 推送到GitHub.sh
```

#### 2. 或者手动执行

```bash
cd /Users/zwj/股票分析

# 初始化 Git（如果还没有）
git init

# 添加所有文件
git add .

# 提交
git commit -m "Initial commit: 大牛股分析系统"

# 添加远程仓库
git remote add origin https://github.com/jet20002025-hash/daniugu.git

# 推送到 GitHub
git branch -M main
git push -u origin main
```

**注意**：如果提示需要认证，可以使用：
- GitHub Personal Access Token（推荐）
- 或 GitHub 用户名和密码

### 第二步：在服务器上部署

#### 1. SSH 连接到服务器

```bash
ssh root@你的服务器IP
```

#### 2. 上传部署脚本到服务器

在本地执行：

```bash
scp 服务器部署脚本_完整版.sh root@你的服务器IP:/root/
```

#### 3. 在服务器上执行部署脚本

```bash
# SSH 连接服务器后
cd /root
chmod +x 服务器部署脚本_完整版.sh
bash 服务器部署脚本_完整版.sh
```

#### 4. 或者手动部署

按照 `阿里云服务器部署指南.md` 中的步骤操作。

### 第三步：配置域名解析

#### 在阿里云 DNS 解析中添加记录

1. 登录 [阿里云控制台](https://ecs.console.aliyun.com)
2. 进入 **域名** → **域名解析**
3. 选择域名 `daniugu.online`
4. 添加 A 记录：

```
记录类型: A
主机记录: @
记录值: 你的服务器IP地址
TTL: 600
```

5. 添加 www 子域名（可选）：

```
记录类型: A
主机记录: www
记录值: 你的服务器IP地址
TTL: 600
```

6. 等待 DNS 生效（通常 5-30 分钟）

### 第四步：配置 SSL 证书（HTTPS）

#### 方式一：使用 Certbot（Let's Encrypt 免费证书）

```bash
# 在服务器上执行

# 安装 Certbot
# CentOS/RHEL
yum install certbot python3-certbot-nginx -y

# Ubuntu/Debian
apt install certbot python3-certbot-nginx -y

# 获取证书
certbot --nginx -d daniugu.online -d www.daniugu.online

# 自动续期测试
certbot renew --dry-run
```

#### 方式二：使用阿里云 SSL 证书

1. 在阿里云申请或购买 SSL 证书
2. 下载证书文件
3. 配置到 Nginx

### 第五步：验证部署

1. **检查服务状态**：

```bash
systemctl status stock-analyzer
systemctl status nginx
```

2. **访问测试**：

- HTTP: http://daniugu.online
- HTTPS: https://daniugu.online（配置 SSL 后）

3. **检查日志**：

```bash
# 应用日志
tail -f /var/www/stock-analyzer/logs/error.log

# Nginx 日志
tail -f /var/log/nginx/stock-analyzer-access.log
```

## 🔄 更新代码

### 方式一：在服务器上直接更新

```bash
# SSH 连接到服务器
ssh root@你的服务器IP

# 进入应用目录
cd /var/www/stock-analyzer

# 拉取最新代码
git pull origin main

# 如果有新的依赖，安装
source venv/bin/activate
pip install -r requirements.txt

# 重启服务
systemctl restart stock-analyzer
```

### 方式二：使用自动部署脚本

创建更新脚本：

```bash
cat > /var/www/stock-analyzer/update.sh << 'EOF'
#!/bin/bash
cd /var/www/stock-analyzer
git pull origin main
source venv/bin/activate
pip install -r requirements.txt
systemctl restart stock-analyzer
echo "更新完成！"
EOF

chmod +x /var/www/stock-analyzer/update.sh
```

使用：

```bash
/var/www/stock-analyzer/update.sh
```

## 🌍 配置 Cloudflare（可选但推荐）

### 为什么推荐 Cloudflare？

1. **CDN 加速**：全球节点，访问更快
2. **DDoS 防护**：自动防护攻击
3. **免费 SSL**：自动配置 HTTPS
4. **缓存优化**：减少服务器负载

### 配置步骤

1. **在 Cloudflare 添加域名**
   - 登录 [Cloudflare](https://dash.cloudflare.com)
   - 点击 "Add a Site"
   - 输入 `daniugu.online`
   - 选择免费计划

2. **获取 Cloudflare DNS 服务器**
   - Cloudflare 会提供两个 DNS 服务器地址

3. **在阿里云修改 DNS 服务器**
   - 登录阿里云控制台
   - 进入 **域名** → **域名解析**
   - 找到 `daniugu.online`，点击 **"修改 DNS 服务器"**
   - 将阿里云的 DNS 服务器替换为 Cloudflare 提供的地址

4. **在 Cloudflare 配置 DNS 记录**
   - 进入 Cloudflare Dashboard
   - 进入 **DNS** → **Records**
   - 添加 A 记录：
     ```
     Type: A
     Name: @
     Content: 你的服务器IP
     Proxy: 🟠 Proxied（橙色云朵）
     ```

5. **SSL/TLS 设置**
   - 进入 **SSL/TLS** 设置
   - 选择 **"Full"** 模式
   - 启用 **"Always Use HTTPS"**

## 📊 监控和维护

### 查看服务状态

```bash
# 应用服务
systemctl status stock-analyzer

# Nginx 服务
systemctl status nginx
```

### 查看日志

```bash
# 应用日志
tail -f /var/www/stock-analyzer/logs/error.log
tail -f /var/www/stock-analyzer/logs/access.log

# Nginx 日志
tail -f /var/log/nginx/stock-analyzer-access.log
tail -f /var/log/nginx/stock-analyzer-error.log
```

### 重启服务

```bash
# 重启应用
systemctl restart stock-analyzer

# 重启 Nginx
systemctl restart nginx

# 重启所有服务
systemctl restart stock-analyzer nginx
```

### 检查端口

```bash
# 检查应用端口
netstat -tlnp | grep 5002

# 检查 Nginx 端口
netstat -tlnp | grep :80
netstat -tlnp | grep :443
```

## 🐛 常见问题

### 问题1: 服务启动失败

**检查**：
```bash
systemctl status stock-analyzer
journalctl -u stock-analyzer -n 50
```

**可能原因**：
- 代码错误
- 依赖未安装
- 端口被占用

### 问题2: 无法访问网站

**检查**：
1. 服务是否运行：`systemctl status stock-analyzer nginx`
2. 防火墙是否开放端口：`firewall-cmd --list-all` 或 `ufw status`
3. DNS 是否解析正确：`dig daniugu.online`
4. Nginx 配置是否正确：`nginx -t`

### 问题3: SSL 证书问题

**解决**：
- 检查 DNS 解析是否正确
- 等待 Certbot 自动配置
- 查看 Certbot 日志：`certbot certificates`

## 📝 部署检查清单

- [ ] 代码已推送到 GitHub
- [ ] 服务器环境已配置（Python、Nginx、Git）
- [ ] 应用已部署到服务器
- [ ] Gunicorn 服务已启动
- [ ] Nginx 已配置并运行
- [ ] 防火墙已开放 80 和 443 端口
- [ ] 域名 DNS 已配置
- [ ] SSL 证书已配置（HTTPS）
- [ ] 网站可以正常访问
- [ ] Cloudflare 已配置（可选）

## 📞 需要帮助？

如果遇到问题：
1. 查看服务日志
2. 检查 Nginx 配置
3. 验证 DNS 解析
4. 查看防火墙设置

---

**部署完成后，访问 https://daniugu.online 即可使用！**



