...
def main():
    ...
    # 检查是否是VIP用户的扫描时间（11:30）
    if current_hour == 11 and current_minute == 30:
        print("[auto_scan_cron] 触发VIP用户自动扫描")
        trigger_auto_scan('premium')
    
    # 检查是否是免费用户的扫描时间（15:00）
    elif current_hour == 15 and current_minute == 0:
        print("[auto_scan_cron] 触发免费用户自动扫描")
        trigger_auto_scan('free')
    
    else:
        print(f"[auto_scan_cron] 当前时间不是自动扫描时间点")
        print(f"[auto_scan_cron] VIP扫描时间: 11:30, 免费扫描时间: 15:00")
