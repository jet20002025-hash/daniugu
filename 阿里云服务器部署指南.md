...
### 3. 使用 GitHub Webhook（可选）

可以配置 GitHub Webhook，代码推送后自动部署。
...
1. **修改 SSH 端口**（避免默认 22 端口）
2. **使用密钥登录**（禁用密码登录）
3. **配置防火墙**（只开放必要端口）
4. **定期更新系统**（`yum update` 或 `apt update`）
5. **使用强密码**
...
**下一步**：告诉我你的服务器信息（操作系统、IP 地址），我可以帮你创建具体的部署脚本！
