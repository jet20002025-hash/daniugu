...
def main():
    ...
    print(f"   - 训练策略：循环训练直到所有股票匹配度都达到1.0")
    ...
    # 步骤4: 验证并调整（循环直到所有股票匹配度达到1.0）
    print("\n" + "=" * 80)
    print("🔍 步骤4: 验证匹配度（循环训练直到所有股票匹配度达到1.0）")
    print("=" * 80)
    ...
    while iteration < max_iterations and not all_perfect:
        iteration += 1
        print(f"\n{'='*80}")
        print(f"🔄 第 {iteration} 次验证（最多 {max_iterations} 次）")
        print(f"{'='*80}")
        
        # 验证匹配度
        all_perfect, match_scores, failed_stocks = test_all_stocks_match_score(analyzer, target_stocks)
        ...
        print(f"\n⚠️ 有 {len(failed_stocks)} 只股票的匹配度未达到1.0，准备重新训练...")
        print(f"   失败的股票: {', '.join(failed_stocks)}")
        print(f"   匹配度详情:")
