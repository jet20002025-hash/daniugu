...
def reanalyze_stocks():
    ...
    for stock_code, stock_name in stocks_to_analyze:
        ...
        if not result.get('success'):
            print(f"❌ 添加股票失败: {result.get('message', '')}")
            continue
        ...
        if start_idx is not None:
            ...
            print(f"\n📊 买点前后各5周的数据:")
            print("-" * 80)
            start_range = max(0, start_idx - 5)
            end_range = min(len(weekly_df), start_idx + 6)
        ...
        if buy_points_result.get('success'):
            ...
            if buy_points:
                print(f"\n前5个买点:")
                for i, bp in enumerate(buy_points[:5], 1):
                    match_score = bp.get('匹配度', 0)
                    buy_date = bp.get('日期', '')
                    buy_price = bp.get('价格', 0)
                    is_best = bp.get('是否最佳买点', False)
                    gain_10w = bp.get('买入后10周涨幅', 0)
                    
                    print(f"   {i}. {buy_date} | 价格: {buy_price:.2f} | 匹配度: {match_score:.3f} | 10周涨幅: {gain_10w:.2f}% | {'⭐ 最佳买点' if is_best else ''}")
